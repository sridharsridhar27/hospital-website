...
import { ArrowLeft, Phone, X, UserCheck, Sparkles, ChevronRight, Award, Stethoscope } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { getTeamMembers } from '../api/team';

// Optimized Progressive Image Component
const ProgressiveImage = memo(({ src, alt, className, width, height, fallbackSrc = '/placeholder-avatar.png' }) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-3 pt-2">
        ...
        >
          <span className="z-10 transition-colors duration-300">View Full Bio & Details</span>
          <div className="z-10 flex h-7 w-7 items-center justify-center rounded-xl bg-white/80 text-[#0E5C4E] shadow-sm transition-all duration-300 group-hover/btn:bg-white/20 group-hover/btn:text-white">
            <ChevronRight size={15} className="transition-transform duration-300 group-hover/btn:translate-x-0.5" />
          </div>
        </button>
      </div>
    </motion.article>
        ...
        <div className="grid md:grid-cols-12">
          ...
          <div className="relative h-72 w-full bg-[#E8F4EF] md:col-span-5 md:h-auto min-h-[320px]">
            ...
            <div className="absolute bottom-4 left-4 rounded-xl bg-white/90 px-3 py-1.5 backdrop-blur-md md:hidden">
              <p className="text-xs font-bold text-[#0E5C4E]">{selectedMember.designation}</p>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <div className="max-h-52 overflow-y-auto pr-3 text-xs sm:text-sm leading-relaxed text-[#62726C] [scrollbar-width:thin] [scrollbar-color:#0E5C4E/20_transparent]">
                  <p className="whitespace-pre-line">{selectedMember.about}</p>
                </div>
              </div>
            </div>
...
function TeamSkeleton() {
  return (
    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
      {[1, 2, 3].map((index) => (
        <div key={index} className="animate-pulse rounded-[2.5rem] border border-[#E5E9E5] bg-white p-3 shadow-sm">
    ...
    const loadTeamMembers = async () => {
      try {
        ...
        if (err.name !== 'CanceledError' && isMounted) {
          ...
          setError('Unable to load our team members at this moment.');
        }
      } finally {
        if (isMounted) {
          setLoading(false);
